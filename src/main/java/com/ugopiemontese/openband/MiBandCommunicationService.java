...
import com.ugopiemontese.openband.bluetooth.BLEAction;
import com.ugopiemontese.openband.bluetooth.BLETask;
import com.ugopiemontese.openband.bluetooth.WaitAction;
import com.ugopiemontese.openband.bluetooth.WriteAction;
...
public class MiBandCommunicationService extends Service
{
    ...
    private BroadcastReceiver mGoalReceiver = new BroadcastReceiver()
    {
        @Override
        public void onReceive(Context context, final Intent intent)
        {
            final int goal = intent.getIntExtra("goal", 5000);
            setGoal(goal);
        }
    };
	...
	@Override public void onCreate()
	{
        ...
        LocalBroadcastManager.getInstance(this).registerReceiver(mGoalReceiver, new IntentFilter("goal"));
	...
	private void vibrate(final long duration)
	{
		...
		{
			Toast.makeText(getApplicationContext(), "Vibrate: MiBand not paired", Toast.LENGTH_SHORT).show();
		}
	}
    ...
    private void setGoal(int goal)
    {
        ...
        list.add(new WriteAction(MiBandConstants.UUID_CHARACTERISTIC_CONTROL_POINT, new byte[]{ (byte) 5, (byte) 0, (byte) (goal & 0xff ), (byte) ((goal >>> 8) & 0xff) }));
        ...
        {
            Toast.makeText(getApplicationContext(), "SetGoal: MiBand not paired", Toast.LENGTH_SHORT).show();
        }
    }
	...
	private void setColor(byte r, byte g, byte b, boolean display)
	{
		...
		{
			Toast.makeText(getApplicationContext(), "SetColor: MiBand not paired", Toast.LENGTH_SHORT).show();
		}
	}
